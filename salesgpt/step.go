...
func (salesGPT *SalesGPT) StepWithOptions(ctx context.Context, options StepOptions) (StepResult, error) {
	...
	workflow.AddNode(responseContextNodeName, "Build customer response context", salesGPT.responseContextNode)
	workflow.AddNode(responseNodeName, "Generate customer response bubbles", salesGPT.responseNode)
	...
	workflow.AddEdge(missingToolQuestionPlanNodeName, responseContextNodeName)
	workflow.AddEdge(responseContextNodeName, responseNodeName)
